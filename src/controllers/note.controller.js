...
const sortNotes = async (req, res) => {
  try {
    ...
    res.status(200).json({ success: true, message: `Notes sorted by ${sortBy} in ${order === 1 ? "ascending" : "descending"} order`, count: notes.length, data: notes });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message, data: null });
  }
};
...
const sortPinnedNotes = async (req, res) => {
  try {
    ...
    res.status(200).json({ success: true, message: `Pinned notes sorted by ${sortBy} in ${order === 1 ? "ascending" : "descending"} order`, count: notes.length, data: notes });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message, data: null });
  }
};
...
module.exports = {
  ...
  getNotesByCategory, getNotesByStatus, getNoteSummary, filterNotes, getPinnedNotes, filterByCategory, filterByDateRange,
  paginateNotes, paginateByCategory, sortNotes, sortPinnedNotes
};